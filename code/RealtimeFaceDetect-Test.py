...
nose_cascade = cv2.CascadeClassifier('E:\\opencv\\opencv\\sources\\data\\haarcascades\\haarcascade_mcs_nose.xml')
mouse_cascade = cv2.CascadeClassifier('E:\\opencv\\opencv\\sources\\data\\haarcascades\\haarcascade_mcs_mouth.xml')
...
while(True):
    ...
    for (x, y, w, h) in faces:
        ...
        roi_gray_up = gray[y:(y+h//2), x:x+w]
        roi_gray_low = gray[(y+h//2):y+h, x:x+w]
        roi_color_up = img[y:(y+h//2), x:x+w]
        roi_color_low = img[(y+h//2):y+h, x:x+w]

        eyes = eye_cascade.detectMultiScale(roi_gray_up)
        for (ex,ey,ew,eh) in eyes:
            cv2.rectangle(roi_color_up,(ex,ey),(ex+ew,ey+eh),(0,255,0),2)
        ...
        for (nx,ny,nw,nh) in noses:
            ...
            cv2.putText(img,'nose',(nx+x,ny+y), font, 0.5, (11,255,255), 1, cv2.LINE_AA)

        mouses = mouse_cascade.detectMultiScale(roi_gray_low)
        for (mx,my,mw,mh) in mouses:
            cv2.rectangle(roi_color_low,(mx,my),(mx+mw,my+mh),(255,255,0),2)
            cv2.putText(img,'mouse',(mx+x,my+y+h//2), font, 0.5, (11,255,255), 1, cv2.LINE_AA)
